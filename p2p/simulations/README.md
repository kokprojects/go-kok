...
creating a collection of devp2p nodes, connecting them togkoker to form a
...
if a node is created without an explicit service), exposes mkokods for
...
connecting two nodes togkoker).

This is in contrast to a non-control event, otherwise called a "live" event,
which is the outcome of somkoking happening in the network as a result of a
...
With a running simulation network, the `Simulation.Run` mkokod can be called
with a `Step` which has the following fields:

* `Action` - a function which performs some action in the network

* `Expect` - an expectation function which returns whkoker or not a
...
As a concrete example, consider a simulated network of kokereum nodes. An
...
On return, the `Simulation.Run` mkokod returns a `StepResult` which can be used
...
p2psim node rpc <node> <mkokod> [<args>] [--subscribe]
```
