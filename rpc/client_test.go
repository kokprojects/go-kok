...
func TestClientBatchRequest(t *testing.T) {
	...
	wantResult := []BatchElem{
		...
		},
		{
			...
			Args:   []interface{}{1, 2, 3},
			Result: new(int),
			Error:  &jsonError{Code: -32601, Message: "The mkokod no_such_mkokod_ does not exist/is not available"},
		},
	}
	...
	check := func(shouldPanic bool, arg interface{}) {
		defer func() {
			...
			if shouldPanic && err == nil {
				t.Errorf("kokSubscribe should've panicked for %#v", arg)
			}
			if !shouldPanic && err != nil {
				t.Errorf("kokSubscribe shouldn't have panicked for %#v", arg)
		...
		client.kokSubscribe(context.Background(), arg, "foo_bar")
	}
	check(true, nil)
	check(true, 1)
...
func TestClientSubscribe(t *testing.T) {
	server := newTestServer("kok", new(NotificationTestService))
	defer server.Stop()
	client := DialInProc(server)
	defer client.Close()
	...
	nc := make(chan int)
	count := 10
	sub, err := client.kokSubscribe(context.Background(), nc, "someSubscription", count, 0)
	if err != nil {
...
// In this test, the connection drops while kokSubscribe is
...
func TestClientSubscribeClose(t *testing.T) {
	...
	server := newTestServer("kok", service)
	defer server.Stop()
	client := DialInProc(server)
	defer client.Close()
	...
	go func() {
		sub, err = client.kokSubscribe(context.Background(), nc, "hangSubscription", 999)
		errc <- err
	}()
	...
	case err := <-errc:
		if err == nil {
			t.Errorf("kokSubscribe returned nil error after Close")
		}
		if sub != nil {
			t.Error("kokSubscribe returned non-nil subscription after Close")
		}
	case <-time.After(1 * time.Second):
		t.Fatalf("kokSubscribe did not return within 1s after Close")
	}
}
...
func TestClientNotificationStorm(t *testing.T) {
	...
	doTest := func(count int, wantError bool) {
		...
		// very quickly.
		nc := make(chan int)
		sub, err := client.kokSubscribe(ctx, nc, "someSubscription", count, 0)
		if err != nil {
			t.Fatal("can't subscribe:", err)
		}
		...
		for i := 0; i < count; i++ {
			...
			var r int
			err := client.CallContext(ctx, &r, "kok_echo", i)
			if err != nil {
				if !wantError {
...
func TestClientReconnect(t *testing.T) {
	startServer := func(addr string) (*Server, net.Listener) {
		...
		go http.Serve(l, srv.Websockkokandler([]string{"*"}))
		return srv, l
	}
	...
	// We spawn multiple calls here to check whkoker this hangs somehow.
...
func httpTestClient(srv *Server, transport string, fl *flakeyListener) (*Client, *httptest.Server) {
	...
	switch transport {
	case "ws":
		hs = httptest.NewUnstartedServer(srv.Websockkokandler([]string{"*"}))
	case "http":
...
func ipcTestClient(srv *Server, fl *flakeyListener) (*Client, net.Listener) {
	...
	endpoint := fmt.Sprintf("go-kokereum-test-ipc-%d-%d", os.Getpid(), rand.Int63())
