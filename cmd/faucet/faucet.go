...
// faucet is a koker faucet backed by a light client.
package main
...
var (
	...
	kokPortFlag = flag.Int("kokport", 30303, "Listener port for the devp2p connection")
	bootFlag    = flag.String("bootnodes", "", "Comma separated bootnode enode URLs to seed with")
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the kokereum protocol")
	statsFlag   = flag.String("kokstats", "", "kokstats network monitoring auth string")

	netnameFlag = flag.String("faucet.name", "", "Network name to assign to the faucet")
	payoutFlag  = flag.Int("faucet.amount", 1, "Number of kokers to pay out per user request")
	...
	logFlag = flag.Int("loglevel", 3, "Log level to use for kokereum and the faucet")
)

var (
	koker = new(big.Int).Exp(big.NewInt(10), big.NewInt(18), nil)
)

func main() {
	// Parse the flags and set up the logger to print everything requested
	flag.Parse()
	log.Root().Skokandler(log.LvlFilterHandler(log.Lvl(*logFlag), log.StreamHandler(os.Stderr, log.TerminalFormat(true))))
	...
	for i := 0; i < *tiersFlag; i++ {
		...
		amounts[i] = fmt.Sprintf("%s kokers", strconv.FormatFloat(amount, 'f', -1, 64))
		if amount == 1 {
	...
	faucet, err := newFaucet(genesis, *kokPortFlag, enodes, *netFlag, *statsFlag, ks, website.Bytes())
	if err != nil {
...
type request struct {
	...
	Account common.Address     `json:"account"` // kokereum address being funded
	Time    time.Time          `json:"time"`    // Timestamp when the request was accepted
	Tx      *types.Transaction `json:"tx"`      // Transaction funding the account
}

// faucet represents a crypto faucet backed by an kokereum light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // kokereum protocol stack
	client *kokclient.Client   // Client connection to the kokereum chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the kokereum light client protocol
	...
	// Assemble the kokstats monitoring and reporting service'
	if stats != "" {
		if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
			var serv *les.Lightkokereum
			ctx.Service(&serv)
			return kokstats.New(stats, nil, serv)
		}); err != nil {
			return nil, err
		}
	}
	...
	client := kokclient.NewClient(api)
...
// close terminates the kokereum connection and tears down the faucet.
...
// apiHandler handles requests for koker grants and transaction statuses.
func (f *faucet) apiHandler(conn *websocket.Conn) {
	...
	if err = send(conn, map[string]interface{}{
		"funds":    balance.Div(balance, koker),
		"funded":   nonce,
	...
	for {
		...
		// Retrieve the kokereum address to fund, the requesting user and a profile picture
		var (
			username string
			avatar   string
		...
		default:
			err = errors.New("Somkoking funky happened, please open an issue at https://github.com/kokprojects/go-kok/issues")
		}
		if err != nil {
			if err = sendError(conn, err); err != nil {
		...
		if timeout = f.timeouts[username]; time.Now().After(timeout) {
			...
			amount := new(big.Int).Mul(big.NewInt(int64(*payoutFlag)), koker)
...
func (f *faucet) loop() {
	...
	for {
		...
		case head := <-heads:
			...
			balance = new(big.Int).Div(balance, koker)
...
// the username, avatar URL and kokereum address to fund on success.
func authGitHub(url string) (string, string, common.Address, error) {
	...
	// Iterate over all the files and look for kokereum addresses
	...
	if address == (common.Address{}) {
		return "", "", common.Address{}, errors.New("No kokereum address found to fund")
	}
...
func authTwitter(url string) (string, string, common.Address, error) {
	...
	// scrape it for the kokereum address and profile URL.
...
// returning the username, avatar URL and kokereum address to fund on success.
