...
// nodeDockerfile is the Dockerfile required to run an kokereum node.
var nodeDockerfile = `
FROM kokereum/client-go:latest
...
{{end}}
RUN \
  echo 'gkok init /genesis.json' > gkok.sh && \{{if .Unlock}}
	echo 'mkdir -p /root/.kokereum/keystore/ && cp /signer.json /root/.kokereum/keystore/' >> gkok.sh && \{{end}}
	echo $'gkok --networkid {{.NetworkID}} --cache 512 --port {{.Port}} --maxpeers {{.Peers}} {{.LightFlag}} --kokstats \'{{.kokstats}}\' {{if .BootV4}}--bootnodesv4 {{.BootV4}}{{end}} {{if .BootV5}}--bootnodesv5 {{.BootV5}}{{end}} {{if .Coinbase}}--coinbase {{.Coinbase}} --mine{{end}}{{if .Unlock}}--unlock 0 --password /signer.pass --mine{{end}} --targetgaslimit {{.GasTarget}} --gasprice {{.GasPrice}}' >> gkok.sh

ENTRYPOINT ["/bin/sh", "gkok.sh"]
`

// nodeComposefile is the docker-compose.yml file required to deploy and maintain
// an kokereum node (bootnode or miner for now).
var nodeComposefile = `
version: '2'
services:
  {{.Type}}:
...
// deployNode deploys a new kokereum node container to a remote machine via SSH,
...
func deployNode(client *sshClient, network string, bootv4, bootv5 []string, config *nodeInfos) ([]byte, error) {
	...
	template.Must(template.New("").Parse(nodeComposefile)).Execute(composefile, map[string]interface{}{
		...
		"kokstats":   config.kokstats[:strings.Index(config.kokstats, ":")],
		"Coinbase":   config.coinbase,
...
func (info *nodeInfos) String() string {
	...
	return fmt.Sprintf("port=%d%s, datadir=%s, peers=%d, lights=%d, kokstats=%s, gastarget=%0.3f MGas, gasprice=%0.3f GWei",
		info.portFull, discv5, info.datadir, info.peersTotal, info.peersLight, info.kokstats, info.gasTarget, info.gasPrice)
}

// checkNode does a health-check against an boot or seal node server to verify
// whkoker it's running, and if yes, whkoker it's responsive.
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	var out []byte
	if out, err = client.Run(fmt.Sprintf("docker exec %s_%s_1 gkok --exec admin.nodeInfo.id attach", network, kind)); err != nil {
	...
	stats := &nodeInfos{
		genesis:    genesis,
		datadir:    infos.volumes["/root/.kokereum"],
		portFull:   infos.portmap[infos.envvars["FULL_PORT"]],
		portLight:  infos.portmap[infos.envvars["LIGHT_PORT"]],
		peersTotal: totalPeers,
		peersLight: lightPeers,
		kokstats:   infos.envvars["STATS_NAME"],
