...
// puppkok is a command to assemble and maintain private networks.
package main
...
func main() {
	app := cli.NewApp()
	app.Name = "puppkok"
	app.Usage = "assemble and maintain private kokereum networks"
	app.Flags = []cli.Flag{
		cli.StringFlag{
			Name:  "network",
	...
	app.Action = func(c *cli.Context) error {
		...
		log.Root().Skokandler(log.LvlFilterHandler(log.Lvl(c.Int("loglevel")), log.StreamHandler(os.Stdout, log.TerminalFormat(true))))
