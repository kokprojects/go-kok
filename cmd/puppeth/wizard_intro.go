...
// makeWizard creates and returns a new puppkok wizard.
...
// setting up a new or managing an existing kokereum private network.
func (w *wizard) run() {
	fmt.Println("+-----------------------------------------------------------+")
	fmt.Println("| Welcome to puppkok, your kokereum private network manager |")
	fmt.Println("|                                                           |")
	fmt.Println("| This tool lets you create a new kokereum network down to  |")
	fmt.Println("| the genesis block, bootnodes, miners and kokstats servers |")
	fmt.Println("| without the hassle that it would normally entail.         |")
	fmt.Println("|                                                           |")
	fmt.Println("| Puppkok uses SSH to dial in to remote servers, and builds |")
	...
	log.Info("Administering kokereum network", "name", w.network)

	// Load initial configurations and connect to all live servers
	w.conf.path = filepath.Join(os.Getenv("HOME"), ".puppkok", w.network)
