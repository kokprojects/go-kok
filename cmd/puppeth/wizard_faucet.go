...
func (w *wizard) deployFaucet() {
	...
	fmt.Println()
	fmt.Printf("How many kokers to release per request? (default = %d)\n", infos.amount)
	...
	fmt.Println()
	if infos.node.kokstats == "" {
		fmt.Printf("What should the node be called on the stats page?\n")
		infos.node.kokstats = w.readString() + ":" + w.conf.kokstats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.node.kokstats)
		infos.node.kokstats = w.readDefaultString(infos.node.kokstats) + ":" + w.conf.kokstats
	}
