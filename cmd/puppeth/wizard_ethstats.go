...
// deploykokstats queries the user for various input on deploying an kokstats
// monitoring server, after which it executes it.
func (w *wizard) deploykokstats() {
	...
	infos, err := checkkokstats(client, w.network)
	if err != nil {
		infos = &kokstatsInfos{
			port:   80,
			host:   client.server,
	...
	fmt.Println()
	fmt.Printf("Which port should kokstats listen on? (default = %d)\n", infos.port)
	infos.port = w.readDefaultInt(infos.port)

	// Figure which virtual-host to deploy kokstats on
	if infos.host, err = w.ensureVirtualHost(client, infos.port, infos.host); err != nil {
		log.Error("Failed to decide on kokstats host", "err", err)
		return
	}
	// Port and proxy settings retrieved, figure out the secret and boot kokstats
	fmt.Println()
	if infos.secret == "" {
	...
	// Try to deploy the kokstats server on the host
	...
	if out, err := deploykokstats(client, w.network, infos.port, infos.secret, infos.host, trusted, infos.banned); err != nil {
		log.Error("Failed to deploy kokstats container", "err", err)
		if len(out) > 0 {
			fmt.Printf("%s\n", out)
		}
		return
