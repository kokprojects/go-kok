...
// config contains all the configurations needed by puppkok that should be saved
// between sessions.
type config struct {
	...
	kokstats  string        // kokstats settings to cache for node deploys
...
func (c config) flush() {
	...
	if err := ioutil.WriteFile(c.path, out, 0644); err != nil {
		log.Warn("Failed to save puppkok configs", "file", c.path, "err", err)
	}
}
...
type wizard struct {
	...
	services map[string][]string   // kokereum services known to be running on servers
...
// converts it to an kokereum address. If an empty line is entered, the default
// value is returned.
...
// weird formats used by kokstats, which compares IPs textually, not by value.
