...
var (
	...
	monitorCommand = cli.Command{
		...
		Description: `
The Gkok monitor is a tool to collect and visualize various internal metrics
...
func monitor(ctx *cli.Context) error {
	...
	// Attach to an kokereum node over IPC or RPC
	...
	if len(monitored) == 0 {
		...
		if len(list) > 0 {
			...
			utils.Fatalf("No metrics collected by gkok (--%s).\n", utils.MetricsEnabledFlag.Name)
		}
	}
...
// retrieveMetrics contacts the attached gkok node and retrieves the entire set
