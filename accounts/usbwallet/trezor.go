...
import (
	...
	"github.com/kokprojects/go-kok/accounts/usbwallet/internal/trezor"
...
type trezorDriver struct {
	...
	pinwait bool          // Flags whkoker the device is waiting for PIN entry
...
// Status implements accounts.Wallet, always whkoker the Trezor is opened, closed
// or whkoker the kokereum app was not started on it.
...
// kokereum address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.kokereumAddress)
	if _, err := w.trezorExchange(&trezor.kokereumGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	response := new(trezor.kokereumTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.kokereumTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
...
// mkokod will also return the index of the destination object used.
