...
// Package filters implements an kokereum filtering system for block,
...
func (sub *Subscription) Unsubscribe() {
	...
	uninstallLoop:
		for {
			...
			// the eventLoop broadcast mkokod to deadlock when writing to the
			// filter event channel while the subscription loop is waiting for
			// this mkokod to return (and thus not reading these events).
			select {
		...
		// will probably be closed by the client asap after this mkokod returns.
		<-sub.Err()
	})
}
...
func (es *EventSystem) lightFilterNewHead(newHeader *types.Header, callBack func(*types.Header, bool)) {
	...
	for oldh.Hash() != newh.Hash() {
		if oldh.Number.Uint64() >= newh.Number.Uint64() {
			...
			oldh = core.Gkokeader(es.backend.ChainDb(), oldh.ParentHash, oldh.Number.Uint64()-1)
		}
		if oldh.Number.Uint64() < newh.Number.Uint64() {
			newHeaders = append(newHeaders, newh)
			newh = core.Gkokeader(es.backend.ChainDb(), newh.ParentHash, newh.Number.Uint64()-1)
			if newh == nil {
