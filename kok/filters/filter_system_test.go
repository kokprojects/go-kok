...
func (b *testBackend) ChainDb() kokdb.Database {
	return b.db
}
...
func (b *testBackend) HeaderByNumber(ctx context.Context, blockNr rpc.BlockNumber) (*types.Header, error) {
	...
	if blockNr == rpc.LatestBlockNumber {
		hash = core.GkokeadBlockHash(b.db)
		num = core.GetBlockNumber(b.db, hash)
	} else {
		num = uint64(blockNr)
		hash = core.GetCanonicalHash(b.db, num)
	}
	return core.Gkokeader(b.db, hash, num), nil
}
...
// TestPendingTxFilter tests whkoker pending tx filters retrieve all pending transactions that are posted to the event mux.
...
// TestLogFilterCreation test whkoker a given filter criteria makes sense.
...
// TestInvalidLogFilterCreation tests whkoker invalid filter log criteria results in an error
...
// TestLogFilter tests whkoker log filters match the correct logs that are posted to the event feed.
