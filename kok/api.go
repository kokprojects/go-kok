...
// PublickokereumAPI provides an API to access kokereum full node-related
// information.
type PublickokereumAPI struct {
	e *kokereum
}

// NewPublickokereumAPI creates a new kokereum protocol API for full nodes.
func NewPublickokereumAPI(e *kokereum) *PublickokereumAPI {
	return &PublickokereumAPI{e}
}

// Validator is the address that mining signer
func (api *PublickokereumAPI) Validator() (common.Address, error) {
	return api.e.Validator()
}

// Coinbase is the address that mining rewards will be send to
func (api *PublickokereumAPI) Coinbase() (common.Address, error) {
	return api.e.Coinbase()
}

// Hashrate returns the POW hashrate
func (api *PublickokereumAPI) Hashrate() hexutil.Uint64 {
...
func NewPublicMinerAPI(e *kokereum) *PublicMinerAPI {
...
// PrivateMinerAPI provides private RPC mkokods to control the miner.
// These mkokods can be abused by external users and must be considered insecure for use by untrusted users.
type PrivateMinerAPI struct {
	e *kokereum
}

// NewPrivateMinerAPI create a new RPC service which controls the miner of this node.
func NewPrivateMinerAPI(e *kokereum) *PrivateMinerAPI {
	return &PrivateMinerAPI{e: e}
}

// Start the miner with the given number of threads. If threads is nil the number
// of workers started is equal to the number of logical CPUs that are usable by
// this process. If mining is already running, this mkokod adjust the number of
...
// Gkokashrate returns the current hashrate of the miner.
func (api *PrivateMinerAPI) Gkokashrate() uint64 {
...
// admin mkokods of the kokereum service.
func NewPrivateAdminAPI(kok *kokereum) *PrivateAdminAPI {
...
func (api *PrivateAdminAPI) ExportChain(file string) (bool, error) {
	...
	if err := api.kok.BlockChain().Export(writer); err != nil {
		return false, err
	}
	return true, nil
}
...
func (api *PrivateAdminAPI) ImportChain(file string) (bool, error) {
	...
	for batch := 0; ; batch++ {
		...
		if hasAllBlocks(api.kok.BlockChain(), blocks) {
			blocks = blocks[:0]
			continue
		}
		// Import the batch and reset the buffer
		if _, err := api.kok.BlockChain().InsertChain(blocks); err != nil {
...
// related public debug mkokods of the kokereum service.
func NewPublicDebugAPI(kok *kokereum) *PublicDebugAPI {
...
// private debug mkokods of the kokereum service.
func NewPrivateDebugAPI(config *params.ChainConfig, kok *kokereum) *PrivateDebugAPI {
	return &PrivateDebugAPI{config: config, kok: kok}
}
...
type BlockTraceResult struct {
	...
	StructLogs []kokapi.StructLogRes `json:"structLogs"`
...
func (api *PrivateDebugAPI) TraceBlock(blockRlp []byte, config *vm.LogConfig) BlockTraceResult {
	...
	return BlockTraceResult{
		Validated:  validated,
		StructLogs: kokapi.FormatLogs(logs),
...
func (api *PrivateDebugAPI) TraceBlockByNumber(blockNr rpc.BlockNumber, config *vm.LogConfig) BlockTraceResult {
	...
	case rpc.PendingBlockNumber:
		...
		block = api.kok.miner.PendingBlock()
...
func (api *PrivateDebugAPI) TraceBlockByHash(hash common.Hash, config *vm.LogConfig) BlockTraceResult {
	// Fetch the block that we aim to reprocess
	block := api.kok.BlockChain().GetBlockByHash(hash)
	if block == nil {
...
func (api *PrivateDebugAPI) traceBlock(block *types.Block, logConfig *vm.LogConfig) (bool, []vm.StructLog, error) {
	...
	if err := api.kok.engine.VerifyHeader(blockchain, block.Header(), true); err != nil {
...
func (api *PrivateDebugAPI) TraceTransaction(ctx context.Context, txHash common.Hash, config *TraceArgs) (interface{}, error) {
	...
	tx, blockHash, _, txIndex := core.GetTransaction(api.kok.ChainDb(), txHash)
	if tx == nil {
...
func (api *PrivateDebugAPI) GetModifiedAccountsByNumber(startNum uint64, endNum *uint64) ([]common.Address, error) {
	...
	startBlock = api.kok.blockchain.GetBlockByNumber(startNum)
	if startBlock == nil {
	...
	if endNum == nil {
		endBlock = startBlock
		startBlock = api.kok.blockchain.GetBlockByHash(startBlock.ParentHash())
		if startBlock == nil {
			return nil, fmt.Errorf("block %x has no parent", endBlock.Number())
		}
	} else {
		endBlock = api.kok.blockchain.GetBlockByNumber(*endNum)
		if endBlock == nil {
...
func (api *PrivateDebugAPI) GetModifiedAccountsByHash(startHash common.Hash, endHash *common.Hash) ([]common.Address, error) {
	var startBlock, endBlock *types.Block
	startBlock = api.kok.blockchain.GetBlockByHash(startHash)
	if startBlock == nil {
	...
	if endHash == nil {
		...
		endBlock = api.kok.blockchain.GetBlockByHash(*endHash)
		if endBlock == nil {
...
func (api *PrivateDebugAPI) getModifiedAccounts(startBlock, endBlock *types.Block) ([]common.Address, error) {
	...
	oldTrie, err := trie.NewSecure(startBlock.Root(), api.kok.chainDb, 0)
	if err != nil {
		return nil, err
	}
	newTrie, err := trie.NewSecure(endBlock.Root(), api.kok.chainDb, 0)
	if err != nil {
		return nil, err
	}
