...
func testGetNodeData(t *testing.T, protocol int) {
	...
	hashes := []common.Hash{}
	for _, key := range pm.chaindb.(*kokdb.MemDatabase).Keys() {
	...
	statedb, _ := kokdb.NewMemDatabase()
...
// Tests that post kok protocol handshake, DAO fork-enabled clients also execute
...
func testDAOChallenge(t *testing.T, localForked, remoteForked bool, timeout bool) {
	...
	peer, _ := newTestPeer("peer", kok63, pm, true)
	defer peer.close()
