...
// Skokead updates the head hash and total difficulty of the peer.
func (p *peer) Skokead(hash common.Hash, td *big.Int) {
	p.lock.Lock()
	defer p.lock.Unlock()
...
func (p *peer) String() string {
	return fmt.Sprintf("Peer %s [%s]", p.id,
		fmt.Sprintf("kok/%2d", p.version),
	)
}

// peerSet represents the collection of active peers currently participating in
// the kokereum sub-protocol.
type peerSet struct {
