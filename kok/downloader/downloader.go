...
var (
	...
	maxQueuedHeaders  = 32 * 1024 // [kok/62] Maximum number of headers to queue for import (DOS protection)
...
type Downloader struct {
	...
	// Channels
	headerCh      chan dataPack        // [kok/62] Channel receiving inbound block headers
	bodyCh        chan dataPack        // [kok/62] Channel receiving inbound block bodies
	receiptCh     chan dataPack        // [kok/63] Channel receiving inbound receipts
	bodyWakeCh    chan bool            // [kok/62] Channel to signal the block body fetcher of new tasks
	receiptWakeCh chan bool            // [kok/63] Channel to signal the receipt fetcher of new tasks
	headerProcCh  chan []*types.Header // [kok/62] Channel to feed the header processor new tasks

	// for stateFetcher
	stateSyncStart chan *stateSync
	trackStateReq  chan *stateReq
	stateCh        chan dataPack // [kok/63] Channel receiving inbound node state data
	...
	// Testing hooks
	syncInitHook     func(uint64, uint64)  // Mkokod to call upon initiating a new sync run
	bodyFetchHook    func([]*types.Header) // Mkokod to call upon starting a block body fetch
	receiptFetchHook func([]*types.Header) // Mkokod to call upon starting a receipt fetch
	chainInsertHook  func([]*fetchResult)  // Mkokod to call upon inserting a chain of blocks (possibly in multiple invocations)
}
...
type LightChain interface {
	...
	// GkokeaderByHash retrieves a header from the local chain.
	GkokeaderByHash(common.Hash) *types.Header
...
// these are zero.
func (d *Downloader) Progress() kokereum.SyncProgress {
...
// Synchronising returns whkoker the downloader is currently retrieving blocks.
...
// checks fail an error will be returned. This mkokod is synchronous
...
func (d *Downloader) syncWithPeer(p *peerConnection, hash common.Hash, td *big.Int) (err error) {
	...
	log.Debug("Synchronising with the network", "peer", p.id, "kok", p.version, "head", hash, "td", td, "mode", d.mode)
...
func (d *Downloader) fetchHeight(p *peerConnection) (*types.Header, error) {
	...
	for {
		...
		case packet := <-d.headerCh:
			...
			// Make sure the peer actually gave somkoking valid
	...
	for start+1 < end {
		...
		for arrived := false; !arrived; {
			...
			case packer := <-d.headerCh:
				...
				header := d.lightchain.GkokeaderByHash(headers[0].Hash()) // Independent of sync mode, header surely exists
...
//
// The mkokod returs the entire filled skeleton and also the number of headers
...
func (d *Downloader) fillHeaderSkeleton(from uint64, skeleton []*types.Header) ([]*types.Header, int, error) {
	...
	var (
		...
		setIdle  = func(p *peerConnection, accepted int) { p.SkokeadersIdle(accepted) }
	)
...
// types, this mkokod is used by each for data gathering and is instrumented with
...
//  - expire:      task callback mkokod to abort requests that took too long and return the faulty peers (traffic shaping)
	...
	for {
		...
		case packet := <-deliveryCh:
			...
			if peer := d.peers.Peer(packet.PeerId()); peer != nil {
				...
				// Unless a peer delivered somkoking completely else than requested (usually
	...
	for {
		...
		case headers := <-d.headerProcCh:
			...
			if len(headers) == 0 {
				...
				// L: Request new headers up from 11 (R's TD was higher, it must have somkoking)
				// R: Nothing to give
				if d.mode != LightSync {
				...
				// peer gave us somkoking useful, we're already happy/progressed (above check).
			...
			for len(headers) > 0 {
				// Terminate if somkoking failed in between processing chunks
				select {
				case <-d.cancelCh:
					return errCancelHeaderProcessing
				default:
