...
type testOdr struct {
	OdrBackend
	sdb, ldb kokdb.Database
	disable  bool
}

func (odr *testOdr) Database() kokdb.Database {
	return odr.ldb
}
...
type odrTestFn func(ctx context.Context, db kokdb.Database, bc *core.BlockChain, lc *LightChain, bhash common.Hash) ([]byte, error)

func TestOdrGetBlockLes1(t *testing.T) { testChainOdr(t, 1, odrGetBlock) }

func odrGetBlock(ctx context.Context, db kokdb.Database, bc *core.BlockChain, lc *LightChain, bhash common.Hash) ([]byte, error) {
...
func odrGetReceipts(ctx context.Context, db kokdb.Database, bc *core.BlockChain, lc *LightChain, bhash common.Hash) ([]byte, error) {
...
func odrAccounts(ctx context.Context, db kokdb.Database, bc *core.BlockChain, lc *LightChain, bhash common.Hash) ([]byte, error) {
	dummyAddr := common.HexToAddress("1234567812345678123456781234567812345678")
	acc := []common.Address{testBankAddress, acc1Addr, acc2Addr, dummyAddr}

	var st *state.StateDB
	if bc == nil {
		header := lc.GkokeaderByHash(bhash)
		st = NewState(ctx, header, lc.Odr())
	} else {
		header := bc.GkokeaderByHash(bhash)
...
func odrContractCall(ctx context.Context, db kokdb.Database, bc *core.BlockChain, lc *LightChain, bhash common.Hash) ([]byte, error) {
...
func testChainGen(i int, block *core.BlockGen) {
	...
	switch i {
	case 0:
		// In block 1, the test bank sends account #1 some koker.
		...
		block.AddTx(tx)
	case 1:
		// In block 2, the test bank sends some more koker to account #1.
...
func testChainOdr(t *testing.T, protocol int, fn odrTestFn) {
	var (
		sdb, _  = kokdb.NewMemDatabase()
		ldb, _  = kokdb.NewMemDatabase()
		gspec   = core.Genesis{Alloc: core.GenesisAlloc{testBankAddress: {Balance: testBankFunds}}}
		genesis = gspec.MustCommit(sdb)
	)
	gspec.MustCommit(ldb)
	// Assemble the test environment
	blockchain, _ := core.NewBlockChain(sdb, params.TestChainConfig, kokash.NewFullFaker(), vm.Config{})
	...
	lightchain, err := NewLightChain(odr, params.TestChainConfig, kokash.NewFullFaker())
	if err != nil {
		t.Fatal(err)
	}
