...
func TestDAOForkRangeExtradata(t *testing.T) {
	...
	proBc, _ := NewBlockChain(proDb, &proConf, kokash.NewFaker(), vm.Config{})
	defer proBc.Stop()

	conDb, _ := kokdb.NewMemDatabase()
	...
	conBc, _ := NewBlockChain(conDb, &conConf, kokash.NewFaker(), vm.Config{})
	defer conBc.Stop()
	...
	for i := int64(0); i < params.DAOForkExtraRange.Int64(); i++ {
		...
		db, _ = kokdb.NewMemDatabase()
		gspec.MustCommit(db)
		bc, _ := NewBlockChain(db, &conConf, kokash.NewFaker(), vm.Config{})
		defer bc.Stop()
		...
		db, _ = kokdb.NewMemDatabase()
		gspec.MustCommit(db)
		bc, _ = NewBlockChain(db, &proConf, kokash.NewFaker(), vm.Config{})
		defer bc.Stop()
