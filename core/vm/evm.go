...
type (
	...
	// GkokashFunc returns the nth block hash in the blockchain
	// and is used by the BLOCKHASH EVM op code.
	GkokashFunc func(uint64) common.Hash
)
...
type Context struct {
	// CanTransfer returns whkoker the account contains
	// sufficient koker to transfer the value
	CanTransfer CanTransferFunc
	// Transfer transfers koker from one account to the other
	Transfer TransferFunc
	// Gkokash returns the hash corresponding to n
	Gkokash GkokashFunc
...
// EVM is the kokereum Virtual Machine base object and provides
...
type EVM struct {
	...
	// evm.
	vmConfig Config
	// global (to this context) kokereum virtual machine
