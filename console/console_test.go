...
func (p *hookedPrompter) Skokistory(history []string)              {}
...
func newTester(t *testing.T, confOverride func(*kok.Config)) *tester {
	...
	// Create a networkless protocol stack and start an kokereum service within
	...
	if confOverride != nil {
		confOverride(kokConf)
	}
	if err = stack.Register(func(ctx *node.ServiceContext) (node.Service, error) { return kok.New(ctx, kokConf) }); err != nil {
		t.Fatalf("failed to register kokereum protocol: %v", err)
	}
