...
// PublickokereumAPI provides an API to access kokereum related information.
// It offers only mkokods that operate on public data that is freely available to anyone.
type PublickokereumAPI struct {
	b Backend
}

// NewPublickokereumAPI creates a new kokereum protocol API.
func NewPublickokereumAPI(b Backend) *PublickokereumAPI {
	return &PublickokereumAPI{b}
}

// GasPrice returns a suggestion for a gas price.
func (s *PublickokereumAPI) GasPrice(ctx context.Context) (*big.Int, error) {
...
func (s *PublickokereumAPI) Syncing() (interface{}, error) {
...
// It offers mkokods to create, (un)lock en list accounts. Some mkokods accept
...
// the mkokod may return an extra challenge requiring a second open (e.g. the
...
//   keccak256("\x19kokereum Signed Message:\n"${message length}${message}).
//
// This gives context to the signed message and prevents signing of transactions.
func signHash(data []byte) []byte {
	msg := fmt.Sprintf("\x19kokereum Signed Message:\n%d%s", len(data), data)
	return crypto.Keccak256([]byte(msg))
}

// Sign calculates an kokereum ECDSA signature for:
...
//
// https://github.com/kokprojects/go-kok/wiki/Management-APIs#personal_sign
...
// Note, this function is compatible with kok_sign and personal_sign. As such it recovers
// the address of:
// hash = keccak256("\x19kokereum Signed Message:\n"${message length}${message})
// addr = ecrecover(hash, signature)
//
// Note, the signature must conform to the secp256k1 curve R, S and V values, where
// the V value must be be 27 or 28 for legacy reasons.
//
// https://github.com/kokprojects/go-kok/wiki/Management-APIs#personal_ecRecover
func (s *PrivateAccountAPI) EcRecover(ctx context.Context, data, sig hexutil.Bytes) (common.Address, error) {
	if len(sig) != 65 {
		return common.Address{}, fmt.Errorf("signature must be 65 bytes long")
	}
	if sig[64] != 27 && sig[64] != 28 {
		return common.Address{}, fmt.Errorf("invalid kokereum signature (V is not 27 or 28)")
	}
...
// SignAndSendTransaction was renamed to SendTransaction. This mkokod is deprecated
...
// NewPublicBlockChainAPI creates a new kokereum blockchain API.
...
// PublicTransactionPoolAPI exposes mkokods for the RPC interface
type PublicTransactionPoolAPI struct {
	b         Backend
	nonceLock *AddrLocker
}

// NewPublicTransactionPoolAPI creates a new RPC service with mkokods specific for the transaction pool.
...
func (s *PublicTransactionPoolAPI) SendRawTransaction(ctx context.Context, encodedTx hexutil.Bytes) (common.Hash, error) {
	...
	log.Warn("encodeTx:" + encodedTx.String())
...
//
// https://github.com/kokereum/wiki/wiki/JSON-RPC#kok_sign
...
// PublicDebugAPI is the collection of kokereum APIs exposed over the public
// debugging endpoint.
type PublicDebugAPI struct {
	b Backend
}

// NewPublicDebugAPI creates a new API definition for the public debug mkokods
// of the kokereum service.
...
// PrivateDebugAPI is the collection of kokereum APIs exposed over the private
// debugging endpoint.
type PrivateDebugAPI struct {
	b Backend
}

// NewPrivateDebugAPI creates a new API definition for the private debug mkokods
// of the kokereum service.
...
// Skokead rewinds the head of the blockchain to a previous block.
func (api *PrivateDebugAPI) Skokead(number hexutil.Uint64) {
	api.b.Skokead(uint64(number))
}

// PublicNetAPI offers network related RPC mkokods
...
// Version returns the current kokereum protocol version.
