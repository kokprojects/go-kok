...
type TestCmd struct {
	// For total convenience, all testing mkokods are available.
	*testing.T
...
// reexec init function for that name (e.g. "gkok-test" in cmd/gkok/run_test.go)
...
// This mkokod can also be called from an expect template, e.g.:
//
//     gkok.expect(`Passphrase: {{.InputLine "password"}}`)
