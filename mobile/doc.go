...
// Package gkok contains the simplified mobile APIs to go-kokereum.
//
// The scope of this package is *not* to allow writing a custom kokereum client
// with pieces plucked from go-kokereum, rather to allow writing native dapps on
...
// from the go-kokereum repository. Please adhere to this scoping to prevent this
// package getting unmaintainable.
//
// Wrapping guidelines:
//
// Every type that is to be exposed should be wrapped into its own plain struct,
// which internally contains a single field: the original go-kokereum version.
// This is needed because gomobile cannot expose named types for now.
//
// Whenever a mkokod argument or a return type is a custom struct, the pointer
...
// a go slice with the mkokods `Size`, `Get` and `Set`. Further slice operations
...
//
// If a mkokod has multiple return values (e.g. some return + an error), those
