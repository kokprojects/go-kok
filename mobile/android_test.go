...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", Gkok.LightScryptN, Gkok.LightScryptP);
		...
		try {
			...
			// data from this mkokod invocation is a JSON encoded, encrypted key-file.
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.kokereum", new NodeConfig());
			node.start();
			...
			kokereumClient ec = node.getkokereumClient();
	...
	// signatures too. Regression test for go-kokereum issue #14599.
...
//
// This mkokod has been adapted from golang.org/x/mobile/bind/java/seq_test.go/runTest
func TestAndroid(t *testing.T) {
	// Skip tests on Windows altogkoker
	...
	workspace, err := ioutil.TempDir("", "gkok-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/kokereum/gkoktest", "libs"} {
		err = os.MkdirAll(dir, os.ModePerm)
		if err != nil {
			t.Fatal(err)
		}
	}
	// Generate the mobile bindings for Gkok and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.kokereum", "github.com/kokprojects/go-kok/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	build.CopyFile(filepath.Join("libs", "gkok.aar"), "gkok.aar", os.ModePerm)

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "kokereum", "gkoktest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
