...
import (
	...
	"github.com/kokprojects/go-kok/logger"
	"github.com/kokprojects/go-kok/p2p"
	"github.com/kokprojects/go-kok/p2p/nat"
	"github.com/kokprojects/go-kok/whisper"
)
...
func main() {
	...
	// Create an kokereum peer to communicate through
	server := p2p.Server{
		PrivateKey: key,
		MaxPeers:   10,
		Name:       name,
	...
	fmt.Println("Starting kokereum peer...")
	if err := server.Start(); err != nil {
		fmt.Printf("Failed to start kokereum peer: %v.\n", err)
		os.Exit(1)
	}

	// Send a message to self to check that somkoking works
