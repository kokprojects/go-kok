...
#
# The cluster contains a bootnode, a gkok node and multiple swarm nodes, with
...
# static gkok configuration
Gkok_IP="192.168.33.3"
Gkok_RPC_PORT="8545"
Gkok_RPC_URL="http://${Gkok_IP}:${Gkok_RPC_PORT}"
...
# start_bootnode starts a bootnode which is used to bootstrap the gkok and
# swarm nodes
start_bootnode() {
...
# start_gkok_node starts a gkok node with --datadir pointing at <base-dir>/gkok
# and a single, unlocked account with password "gkok"
start_gkok_node() {
  local dir="${base_dir}/gkok"
  mkdir -p "${dir}"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get gkok account address"
  fi
  ...
  local args=(
    ...
    --rpc
    --rpcaddr   "${Gkok_IP}"
    --rpcport   "${Gkok_RPC_PORT}"
    --verbosity "6"
  )

  start_node "gkok" "${Gkok_IP}" "$(which gkok)" ${args[@]}
}
...
create_node_network() {
  ...
  # create the vkok pair
  local vkok0="vkok${name}0"
  local vkok1="vkok${name}1"
  ip link add name "${vkok0}" type vkok peer name "${vkok1}"

  # add one end to the bridge
  ip link set dev "${vkok0}" master "${BRIDGE_NAME}"
  ip link set dev "${vkok0}" up

  # add the other end to the namespace, rename it kok0 and give it the ip
  ip link set dev "${vkok1}" netns "${name}"
  ip netns exec "${name}" ip link set dev "${vkok1}" name "kok0"
  ip netns exec "${name}" ip link set dev "kok0" up
  ip netns exec "${name}" ip address add "${ip}/24" dev "kok0"
}
...
create_account() {
  ...
  gkok --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
